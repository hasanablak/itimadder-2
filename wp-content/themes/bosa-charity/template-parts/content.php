...
<?php
	...
	if( !is_sticky() && get_theme_mod( 'archive_post_layout', 'grid' ) == 'list' ){
	...
	}elseif( !is_sticky() && get_theme_mod( 'archive_post_layout', 'grid' ) == 'list-zigzag' ){
		$layout_class = 'list-zigzag';
	}elseif( !is_sticky() && get_theme_mod( 'archive_post_layout', 'grid' ) == 'list-reverse' ){
		$layout_class = 'list-reverse';
	}elseif( !is_sticky() && get_theme_mod( 'archive_post_layout', 'grid' ) == 'single-overlay' ){
		...
		$layout_class = 'single-post';
	}elseif( is_archive() && is_sticky() && get_theme_mod( 'archive_post_layout', 'grid' ) == 'list-zigzag' ){
		$layout_class = 'list-zigzag';
	}elseif( is_archive() && is_sticky() && get_theme_mod( 'archive_post_layout', 'grid' ) == 'list-reverse' ){
		$layout_class = 'list-reverse';
	}elseif( is_archive() && is_sticky() && get_theme_mod( 'archive_post_layout', 'grid' ) == 'single-overlay' ){
		$layout_class = 'single-overlay';
	}
	...
	$image = '';
	if( has_post_thumbnail() && get_theme_mod( 'archive_post_layout', 'grid' ) == 'single-overlay' ){
...
<div class="<?php echo esc_attr( $stickyClass );?>">
	<article id="post-<?php the_ID(); ?>" <?php post_class( $class . ' ' . $layout_class ) ?> style="background-image: url( <?php echo esc_url( $image ); ?> );" >
		...
		if ( has_post_thumbnail() && ( get_theme_mod( 'archive_post_layout', 'grid' ) == 'grid' || get_theme_mod( 'archive_post_layout', 'grid' ) == 'list' || get_theme_mod( 'archive_post_layout', 'grid' ) == 'list-zigzag' || get_theme_mod( 'archive_post_layout', 'grid' ) == 'list-reverse' || get_theme_mod( 'archive_post_layout', 'grid' ) == 'single' ) ) : ?>
	        <figure class="featured-image">
	            <a href="<?php the_permalink(); ?>">
	                <?php
	                if( get_theme_mod( 'sidebar_settings', 'right' ) == 'right' ) {
	                	if ( get_theme_mod( 'archive_post_layout', 'grid' ) == 'grid' || get_theme_mod( 'archive_post_layout', 'grid' ) == 'list' || get_theme_mod( 'archive_post_layout', 'grid' ) == 'list-zigzag' || get_theme_mod( 'archive_post_layout', 'grid' ) == 'list-reverse' ){
